<script lang="ts">
  import { editShareGuests, startCoEdit, stopCoEdit } from '../lib/coedit'
  import { ReadFile, SetEditShareGuestPermission } from '../lib/wails'
  import { IconCopy, IconCheck, IconUsers, IconEdit } from '@tabler/icons-svelte'
  ...
  $effect(() => {
    loading = true; error = ''
    ReadFile(path).then(content => startCoEdit(path, content ?? ''))
      .then(link => { url = link; loading = false })
...
<div class="overlay" onclick={onClose}>
  <div class="panel" role="dialog" aria-modal="true" tabindex="-1" use:modalA11y={onClose} onclick={(e) => e.stopPropagation()}>
    <div class="header">
      <IconEdit size={13} />
      <span class="title">Share for Co-Editing</span>
    ...
    <div class="body">
      ...
      {:else if url}
        ...
        <div class="guests">
          <div class="guests-label"><IconUsers size={12} /> Guests ({guests.length})</div>
          {#if guests.length === 0}
              ...
              <div class="guest-row">
                ...
                <label class="toggle">
                  <input type="checkbox" checked={g.canType}
                         onchange={(e) => SetEditShareGuestPermission(path, g.id, (e.target as HTMLInputElement).checked)} />
                  Can type
                </label>
              </div>
            {/each}
        ...
        <p class="hint">
          New guests start read-only — check "Can type" to let them edit. Text-only view, no syntax highlighting.
          Sync starts from the file's saved content, not unsaved local edits.
        </p>
      {/if}
    </div>
    <div class="footer">
